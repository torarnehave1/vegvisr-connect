...
var buildCorsHeaders = /* @__PURE__ */ __name((origin) => ({
...
var emailSchema = external_exports.object({ email: external_exports.string().email() });
var tokenSchema = external_exports.object({ token: external_exports.string().min(12) });
var logEvent = /* @__PURE__ */ __name((event, data) => {
...
app.get("/health", (c) => {
  return c.json(
    {
      status: "ok",
      service: c.env.WORKER_NAME || "vegvisr-connect-api",
...
app.post("/api/magic-link/request", async (c) => {
  ...
  ).bind(token, parsed.data.email, expiresAt, createdAt).run();
...
app.post("/api/magic-link/verify", async (c) => {
  ...
  ).bind(parsed.data.token, now).first();
  if (!record) {
    return c.json({ error: "Token expired" }, 400, jsonHeaders);
  }
  await c.env.DB.prepare("UPDATE magic_links SET used_at = ? WHERE token = ?").bind(now, parsed.data.token).run();
  ...
  ).bind(sessionToken, record.email, now, expiresAt).run();
...
app.post("/api/sessions", async (c) => {
  const body = await c.req.json();
  const code = external_exports.string().min(1).safeParse(body.code);
  const emailInput = external_exports.string().email().safeParse(body.email);
...
app.post("/api/onboarding/progress", async (c) => {
  ...
  const parsed = external_exports.object({
    ...
    step: external_exports.number().int().min(1).max(3),
...
app.post("/api/onboarding/progress/get", async (c) => {
  ...
  const data = record.data_json ? JSON.parse(record.data_json) : {};
...
app.post("/api/onboarding", async (c) => {
  ...
  ).bind(parsed.data.email, JSON.stringify(parsed.data.data), submittedAt).run();
  await c.env.DB.prepare("DELETE FROM onboarding_progress WHERE user_email = ?").bind(parsed.data.email).run();
